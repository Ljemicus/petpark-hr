...
for r in "${ROUTES[@]}"; do
  ...
  # Count executable scripts only; JSON-LD is data, not a browser-executed script.
  executable_scripts=$(printf '%s' "$html" | grep -o '<script[^>]*>' | grep -vi 'type="application/ld+json"' || true)
  total=$(printf '%s' "$executable_scripts" | grep -o '<script' | wc -l | tr -d ' ')
  nonced=$(printf '%s' "$executable_scripts" | grep -o 'nonce="' | wc -l | tr -d ' ')
  integ=$(printf '%s' "$executable_scripts" | grep -o 'integrity="' | wc -l | tr -d ' ')
