...
const STATIC_SRI_CSP_ROUTES = new Set(['/']);

function isStaticSriCspRoute(pathname: string): boolean {
  const normalized = pathname.endsWith('/') && pathname !== '/' ? pathname.slice(0, -1) : pathname;
  if (STATIC_SRI_CSP_ROUTES.has(normalized)) return true;
  if (normalized.endsWith('/en')) {
    return STATIC_SRI_CSP_ROUTES.has(normalized.slice(0, -3));
  }
  return false;
}
...
export async function proxy(request: NextRequest) {
  ...
  // Fast path for statically-served public routes: they are prerendered and
  // cached, so they must use the staticBuild CSP (SRI-backed), NOT a per-request
  // nonce that their frozen HTML cannot carry.
  if (isStaticSriCspRoute(pathname)) {
